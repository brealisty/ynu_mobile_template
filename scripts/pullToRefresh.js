...
var refresher = {
    ...
    init: function (parameter) {
        ...
        var list = wrapper.querySelector("#" + parameter.id + " .box");
    ...
    scrollIt: function (parameter, pullDownEl, pullDownOffset, pullUpEl, pullUpOffset) {
        eval(parameter.id + "= new iScroll(parameter.id, {useTransition: true,vScrollbar: false,topOffset: pullDownOffset,onRefresh: function () {refresher.onRelease(this,pullDownEl,pullUpEl);},onScrollMove: function () {refresher.onScrolling(this,pullDownEl,pullUpEl,pullUpOffset);},onScrollEnd: function () {refresher.onPulling(this,pullDownEl,parameter.pullDownAction,pullUpEl,parameter.pullUpAction);},})");
    ...
    onPulling: function (e,pullDownEl, pullDownAction, pullUpEl, pullUpAction) {
        ...
        var pullH= $(".box").height()+40-$(window).height(),
            scrollH=$(document).height()-$(window).height(),
            temp=(scrollH/pullH)* (-e.y-40);
        //调整屏幕滚动条滚动
        if(temp>pullH){temp=pullH};
        $("body").animate({scrollTop:temp},"slow");
        console.log(pullH,temp,$(window).scrollTop());
